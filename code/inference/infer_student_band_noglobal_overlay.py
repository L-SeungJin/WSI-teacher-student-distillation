...
CKPT_PATH = "/workspace/my_exp/student_ckpt_band_noglobal/best_student.pt"
...
class LocalBandBlock(nn.Module):
...
class StudentNetNoGlobal(nn.Module):
    def __init__(self):
        super().__init__()
        d_model = 512
        ...
        self.local1 = LocalBandBlock(d_model, radius=2, sim_beta=1.0)
        self.local2 = LocalBandBlock(d_model, radius=2, sim_beta=1.0)
...
model = StudentNetNoGlobal().to(DEVICE)
...
for d in slide_dirs:
    ...
    out_path = f"{d}/{sid}_overlay_student_band_noglobal.png"
