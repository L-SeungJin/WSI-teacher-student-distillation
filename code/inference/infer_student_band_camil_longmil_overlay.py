...
# ======================
# 무조건 CPU 사용 (CUDA OOM 방지)
DEVICE = "cpu"

WSI_ROOT = "/data2/RAW_DATA/GC_WSI_SNU/AGC_HE-30"
DATA_ROOT = "/workspace/GC_WSI_Results_MoE_UNI"
CKPT_PATH = "/workspace/my_exp/student_ckpt_band_camil_longmil/best_student.pt"
...
def make_thumbnail(wsi, max_size=2000):
    ...
    # 진짜 필요한 scale: level0 -> final thumbnail
    scale_x = new_w / W0
    scale_y = new_h / H0
...
class LocalBandSimilarityBlock(nn.Module):
    ...
    def forward(self, x, grid):
        ...
        # CPU-friendly: N x N 전체 mask 만들지 않고,
        # patch별로 local window neighbor를 직접 찾음
        for i in range(N):
            gx = grid[i, 0]
            gy = grid[i, 1]
            ...
            sim = F.cosine_similarity(h[i:i+1], h[nbr], dim=1).unsqueeze(0)
...
class StudentNet(nn.Module):
    def __init__(self):
        ...
        self.local1 = LocalBandSimilarityBlock(d_model, radius=2, sim_beta=1.0)
        self.local2 = LocalBandSimilarityBlock(d_model, radius=2, sim_beta=1.0)
...
for d in slide_dirs:
    ...
    out_path = f"{d}/{sid}_overlay_student_band_camil_longmil.png"
